#include "newTokenNode.h"

#include "mem.h"
#include "handleErrors.h"

#include <stdio.h>
#include <stdlib.h>

Token* token_create(void) {
        /* ALlocate space for the token object */
        Token* tok = safeMalloc(sizeof(*tok), FATAL);

        /* Allocate space for the token's contents */
        tok->text = str_create(FATAL);

        /* Begin as a loose node */
        tok->next = NULL;       

        /* Initialize Metadata */
        tok->lineNo = 0;
        tok->tType = TOK_WORD;
        tok->kType = KEY_NOTYPE;
        tok->cqTypes = str_create(FATAL);
        tok->untermQ = FALSE;

        /* Return the initialized token */
        return tok;
}

void token_destroy(Token* tok) {
        /* If tok is already free and NULL, attempting to destroy the strings 
         * inside will cause a segfault, so we'll just return without doing 
         * anything instead */
        if (!tok) return;

        /* Free and reset the token's contents */
        str_destroy(tok->text);
        tok->text = NULL;

        /* Detatch from the list */
        tok->next = NULL;

        /* Free and reset metadata */
        tok->lineNo = 0;
        tok->tType = TOK_WORD;
        tok->kType = KEY_NOTYPE;
        str_destroy(tok->cqTypes);
        tok->cqTypes = NULL;
        tok->untermQ = FALSE;

        /* Free the token object */
        safeFree(tok);
}

void token_appendChar(Token* tok, char* src, int pos, QType qType) {
        /* Add the character to the token's string */
        str_append(tok->text, src, pos, FATAL);

        /* Add context marker to the character contexts string */
        switch (qType) {
                case Q_NOTYPE: /* not in quotes */
                        str_append(tok->cqTypes, "-", 0, FATAL);
                        return;
                case Q_SINGLE: /* '...' */
                        str_append(tok->cqTypes, "S", 0, FATAL);
                        return;
                case Q_DOUBLE: /* "..." */
                        str_append(tok->cqTypes, "D", 0, FATAL);
                        return;
                case Q_CMDSUB: /* `...` */
                        str_append(tok->cqTypes, "C", 0, FATAL);
                        return;
        }
}

void token_print(Token* tok) {
        char* tokTextPlain = STR_TEXT(tok->text, FATAL);

        printf("%s", tokTextPlain);

        safeFree(tokTextPlain);
}
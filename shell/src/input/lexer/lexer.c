...
TokList* input_lex(char* input) {
        ...
        while (1) {
                ...
                /* Skip comments: When a non-escaped # is detected, advance */
                /* to  the end of the line without creating or adding to a */
                /* token */
                if (input[pos] == '#') {
...
void handle_escape(TokList* toks, char* input, int* pos, Bool* inTok) {
        ...
        /* Special case: Escaping a newline discards both the backslash and */
        /* the newline--will use in batch mode */
        if (input[(*pos) + 1] == '\n') {
                (*pos) += 2;
                return;
        }

        /* Gather only the character following the backslash, not the */
        /* backslash itself, and advance the cursor past both */

...
static char* capture_interactive(void) {
        String* inputBuff = str_create();   /* Store the raw input */
        ...
        while ((c = str_append(inputBuff, NULL, -1)) != EOF) {
        ...
        inputCStr = str_getText(inputBuff);
...
static char* capture_batch(char* srcPath) {
        FILE* file = NULL;              /* Pointer for the batch file stream */
        String* buff = str_create();    /* Buffer to hold raw input */
        char* input = NULL;             /* c-string to return */
        char curr[2] = {'\0','\0'};     /* The current char from the stream */
        char next[2] = {'\0','\0'};     /* The next char from the stream */
